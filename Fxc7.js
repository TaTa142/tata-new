...
async function starts() {
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			const FarhanGans2 = " ~ 𝐂𝐫𝐞𝐚𝐭𝐞𝐝 𝐁𝐲 𝐅"
			const isUrl = (url) => {
			...
			case 'infobot':
				...
				case 'sticker':
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('Bot F', '@fjr_hsb')} ${ran} -o ${ran}`, async (error) => {
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								...
								exec(`webpmux -set exif ${addMetadata('Bot F', authorname)} ${ranw} -o ${ranw}`, async (error) => {
									if (error) return reply(mess.error.stick)
					...
					fs.unlinkSync(ranp)
					if (err) return reply(mess.error.stick)
					exec(`webpmux -set exif ${addMetadata('Bot F', '@fjr_hsb')} ${rano} -o ${rano}`, async (error) => {
					if (error) return reply(mess.error.stick)
